<template>
  <div class="d-flex flex-column mx-4">
    ...
    <div class="mainpage">
      <div
        class="infoblock"
        v-html="$t('regions.long')"
      />
      <div class="row">
        <label
          for="countrylist"
          class="form-label sm-size mb-2"
        >{{ $t('regions.list') }}</label>
        <select
          id="countrylist"
          v-model="country"
...
export default {
  ...
  data: function () {
    return {
      ...
      countries : [
        { code: "nl", description: this.$t('regions.nl') },
        { code: "be", description: this.$t('regions.be') },
        { code: "de", description: this.$t('regions.de') },
        { code: "fr", description: this.$t('regions.fr') } 
      ],

      // placeholder for the search box
      phsearch: this.$t('regions.ph'),
      defsearch: 1,
      ...
      countrydefs : {
        ...
        be : {
          cols: [
            { field: "flag", label: this.$t('regions.flag'), sortable: false, width: "7%",
              image: {
                src: (r) => {
            ...
            { field: "code", label: this.$t('regions.code'), sortable: true, width: "7%",
              parseCol: (s) => { return s.match(/([A-Za-z]{3}[\s]*)/g); },
            },
            { field: "name", label: this.$t('regions.name'), sortable: true, width: "30%",
              link : {
                href: (r) => { return "https://nl.wikipedia.org/wiki/" + r.url; }
              }
            },
            { field: "cat", label: this.$t('regions.cat'), sortable: true, width: "20%" },
            { field: "cap", label: this.$t('regions.cap'), sortable: true, width: "25%" },
            { field: "mun", label: this.$t('regions.mun'),
        ...
        de : {
          cols: [
            ...
            { field: "cap", label: this.$t('regions.cap'), sortable: true, width: "25%" },
            { field: "bundesrat", label: this.$t('regions.bundesrat'),
              sortable: {
                numsort: true
              },
              width: "5%"
            },
            { field: "landkreise", label: this.$t('regions.landkreise'),
              sortable: { numsort: true },
              width: "5%"
            },
            { field: "stadtkreise", label: this.$t('regions.stadtkreise'),
              sortable: { numsort: true },
              width: "5%"
            }
          ],
        ...
        nl : {
          cols: [
            ...
            { field: "cap", label: this.$t('regions.cap'), sortable: true, width: "30%" },
            { field: "mun", label: this.$t('regions.mun'),
              sortable: { numsort: true },
              width: "7%"
            },
            { field: "area", label: this.$t('regions.area'),
              sortable: { numsort: true },
              width: "7%"
            }
          ],
        ...
        fr : {
          cols: [
            ...
            { field: "cat", label: this.$t('regions.cat'), sortable: true, width: "15%" },
            { field: "code", label: this.$t('regions.code'), sortable: true, width: "5%",
              parseCol: (s) => { return s.match(/([A-Za-z]{3}[\s]*)/g); },
            },
            { field: "name", label: this.$t('regions.name'), sortable: true, width: "30%",
              link: {
            ...
            { field: "cap", label: this.$t('regions.cap'), sortable: true, width: "25%" },
            { field: "dep", label: this.$t('regions.dep'),
              sortable: { numsort: true },
              width: "5%"
            },
            { field: "arr", label: this.$t('regions.arr'),
              sortable: { numsort: true },
              width: "5%"
            },
            { field: "canton", label: this.$t('regions.canton'),
              sortable: { numsort: true },
              width: "5%"
            },
            { field: "commune", label: this.$t('regions.mun'),
              sortable: { numsort: true },
              width: "5%"
            },
            { field: "area", label: this.$t('regions.area'),
