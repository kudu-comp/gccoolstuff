...
export default {
  ...
  mounted: function() {
    ...
    if (this.$route.params.cp) {
      this.cipher = this.$route.params.cp;
      this.changeCipher();
    } else {
      this.cipher = "Atbash";
      this.changeCipher();
    ...
    checkKeys: function() {
      ...
      switch (this.cipher) {
        ...
        case 'Bazeries' :
          ...
          break;
        case 'Burrowswheeler' :
          if (key1.length !== 1 || this.alphabet.indexOf(key1[0].toUpperCase()) >= 0) {
        ...
        case 'Nihilisttransposition' : {
          if (this.message.length !== key1.length**2) {
            this.errormsg = this.$t('cperrors.msgmustbesquareofkey'); return false;
          }
          break;
        }
        case 'Onetimepad' : {
          if (this.cleanString(this.message, this.alphabet).length > key1.length) {
            this.errormsg = this.labelkey1 + this.$t('cperrors.keylongerthenmsg'); return false;
          }
          break;
        }
        case 'Polybius' : {
          if (this.alphabet.length !== key1.length**2) {
            this.errormsg = this.labelkey1 + this.$t('cperrors.keymustbesqrroot'); return false;
          }
          if (this.alphabet.length !== key2.length**2) {
            this.errormsg = this.labelkey2 + this.$t('cperrors.keymustbesqrroot'); return false;
          }
          break;
        }
        case 'Portax' : {
        ...
        case 'Substitution' : {
          if (this.alphabet.length !== key1.trim().length) {
            this.errormsg = this.labelkey1 + this.$t('cperrors.keymustbealph'); return false;
          }
          break;
        }
      	case 'Syllabary' : {
          if (!key2.match(/^[0-9]{10}$/g)) {
            this.errormsg = this.labelkey2 + this.$t('cperrors.mustbe10digits'); return false;
          }
          if (!key3.match(/^[0-9]{10}$/g)) {
