<template>
  <div class="d-flex flex-column mx-4">
    ...
    <div class="mainpage">
      ...
      />
      <div class="row">
        <label for="selcp" class="form-label sm-size mb-2">{{ $t('cp.selcipher') }}</label>
        <select
          v-model="selcp"
          id="selcp"
...
export default {
  ...
  mounted: function() {
    ...
    if (this.$route.params.cphr) {
      this.selcp = this.cp.findIndex ( (e) => e.ref == this.$route.params.cp);
    } else {
      this.selcp = 0;
    }
    this.selCipher();
  ...
  methods: {
    ...
    checkKeys: function() {
      ...
      // Check required for all keys
      for (let i = 0; i < this.cp[this.selcp].keys.length; i++) {
        if (this.cp[this.selcp].keys[i].req && this.keys[i].trim() === ""){
      ...
      switch (this.cipher) {
        ...
        case 'bacon' :
          ...
          break;
        case 'bazeries' :
          if (!keys[2].match(/^[0-9]+$/g)) {
            this.errormsg = this.labels[2] + this.$t('cperrors.mustbenumeric'); return false;
          }
          break;
        case 'burrowswheeler' :
          if (keys[0].length !== 1 || this.alphabet.indexOf(key1[0].toUpperCase()) >= 0) {
        ...
        case 'digrafid' :
          ...
          break;
        case 'gromark' :
          if (!keys[1].match(/^[0-9]+$/g)) {
            this.errormsg = this.labels[1] + this.$t('cperrors.mustbenumeric'); return false;
          }
          break;
        case 'gronsfeld' :
          if (!keys[0].match(/^[0-9]+$/g)) {
            this.errormsg = this.labels[0] + this.$t('cperrors.mustbenumeric'); return false;
          }
          break;
        case 'vatsyayana' :
    ...
    selCipher: function() {
      ...
      this.explanation = this.$t("cp." + this.cp[this.selcp].ref + ".info");
      ...
      for (let i = 0; i < this.keys.length; i++) {
        ...
        this.labels[i] = this.$t("cp." + this.cp[this.selcp].ref + ".k" + (i+1));
