...
export function getZoneByLocation(_lat, _lon, _level) {
  var xy = getXYByLocation(_lat, _lon, _level);
  var zone = getZoneByXY(xy.x, xy.y, _level);
  return zone;
}

export function getZoneByCode(_code) {
...
function getXYByCode(code) {
  ...
  var h_dec9 =
    "" +
    (h_key.indexOf(code.charAt(0)) * 30 + h_key.indexOf(code.charAt(1))) +
    code.substring(2);
  if (
    h_dec9.charAt(0).match(/[15]/) &&
    h_dec9.charAt(1).match(/[^125]/) &&
    h_dec9.charAt(2).match(/[^125]/)
  ) {
  ...
  let h_decx = new Array();
  let h_decy = new Array();
